...
function canAccess(user, permission) {
  const perms = {
    admin: ['config','mapa','inventario','ordenes','ai_entry','reportes','usuarios'],
    supervisor: ['inventario','ordenes','reportes','mapa_asignado'],
    cliente: ['inventario','ordenes','reportes'],
    operador: ['inventario','ordenes','ai_entry','mapa_asignado'],
  };
...
function getNavItems(user) {
  const all = [
    ...
    { id:'usuarios',   label:'Usuarios',      icon:'👥', href:'usuarios.html',   roles:['admin'], section:'admin' },
  ];
  return all.filter(n => n.roles.includes(user?.rol));
}
...
function renderSidebar(currentPage) {
  ...
  let navHTML = mainItems.map(n => `
    <a href="${n.href}" class="nav-item ${currentPage === n.id ? 'active' : ''}">
...
async function getNextSkuSeq(codigo) {
  const monthPrefix = skuMonthPrefix(codigo);
  const { data } = await Q.from('inventario')
    .select('sku')
    .like('sku', monthPrefix + '%');
  const nums = (data || []).map(r => parseInt((r.sku || '').slice(-3)) || 0);
  const next = nums.length ? Math.max(...nums) + 1 : 1;
  return String(next).padStart(3,'0');
}
async function generateSKU(codigo) {
  const seq = await getNextSkuSeq(codigo);
  return codigo + skuDatePart() + seq;
}
