...
async function loginWithCredentials(username, password) {
  ...
  if (!hash) throw new Error('Usuario sin contraseña configurada');
  let valid = (hash === password);
  if (!valid) { try { valid = (atob(hash) === password); } catch(e) {} }
  if (!valid) throw new Error('Contraseña incorrecta');
  await sb().from('usuarios').update({ ultimo_acceso: new Date().toISOString() }).eq('id', data.id);
  const userObj = {
    id: data.id, nombre: data.nombre_display || data.nombre,
    username: data.username, rol: data.rol,
    cliente_id: data.cliente_id, almacen_id: data.almacen_id, color: data.color,
  };
  saveUser(userObj);
  return userObj;
}
function logout() { clearUser(); window.location.replace('login.html'); }
function can(action) {
  const u = currentUser();
  if (!u) return false;
