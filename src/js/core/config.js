...
// ── Roles ───────────────────────────────────────────────
// Roles disponibles: admin, supervisor, operador, cliente
// NOTA: el rol "gerente" fue eliminado — no debe usarse
const ROLES = {
     admin:      { label:'Administrador', icon:'👑', color:'#f97316', perms:['all'] },
     supervisor: { label:'Supervisor',    icon:'🏢', color:'#0d2b7a', perms:['view','edit','report'] },
...
function getNavItems(user) {
  ...
  if (esAdmin) return [...base, ...adminItems];
     if (rol === 'supervisor' || rol === 'operador') return [...base, ...staffItems];
     return base;
}
